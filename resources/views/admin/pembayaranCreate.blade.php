        ...
        <form action="{{ route('admin.pembayaran.store') }}" method="post">
            ...
            <div class="mb-3">
                ...
                <input class="form-control" type="text" name="alamat_pengiriman" id="alamat_pengiriman" 
                       value="{{ old('alamat_pengiriman') }}" required>
            </div>
            ...
            <div class="mb-3">
                ...
                <select class="form-select" name="metode_pembayaran" id="metode_pembayaran" required>
                    ...
                    <option value="cash_on_delivery" {{ old('metode_pembayaran') == 'cash_on_delivery' ? 'selected' : '' }}>Cash on Delivery</option>
                </select>
            </div>
            ...
            <div class="mb-3">
                ...
                <input class="form-control" type="datetime-local" name="tanggal_checkout" id="tanggal_checkout" 
                       value="{{ old('tanggal_checkout') ?? now()->format('Y-m-d\TH:i') }}" required>
            </div>

            <button class="btn btn-success w-100" type="submit">Tambah Pembayaran</button>
        </form>
    </div>
</div>
@endsection
